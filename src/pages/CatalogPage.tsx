          ...
          return (
            <Card key={m.model_id} className={featured ? "border-primary/50 bg-primary/5 hover:border-primary transition-colors flex flex-col" : "hover:border-primary/50 transition-colors flex flex-col"}>
              ...
              <CardContent className="pt-0 pb-2 flex-1 flex items-center gap-2 flex-wrap">
                ...
                {isAdmin && (
                  <>
                    <Badge variant="outline" className="text-[10px] gap-1 border-amber-500/50 text-amber-600 dark:text-amber-400" title="Costo real Wavespeed por generación (admin: gratis)">
                      ${(m.base_price ?? 0).toFixed(3)} WS · {adminCost(m.base_price, pricing.creditsPerUsd)}cr
                    </Badge>
                  </>
                )}
              </CardContent>
