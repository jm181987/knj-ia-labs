...
export default async function handler(req, res) {
  ...
  try {
    ...
    const r=await query(`select p.amount_uyu,u.email from payments p join app_users u on u.id=p.user_id where p.mp_response->>'provider'='mercadopago' order by p.created_at desc limit 1`);
