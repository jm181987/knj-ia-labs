...
create type if not exists public.affiliate_status as enum ('pending','approved','rejected','blocked');
create type if not exists public.affiliate_tier as enum ('bronze','silver','gold');
create type if not exists public.commission_status as enum ('pending','approved','paid','reversed','rejected');
create type if not exists public.commission_type as enum ('one_time','recurring');
create type if not exists public.payout_status as enum ('pending','approved','paid','rejected');
...
create index on public.affiliates (code);
...
create index on public.affiliate_payouts (affiliate_id, created_at desc);
...
create index on public.affiliate_commissions (affiliate_id, status, created_at desc);
...
-- ===== RPC: register affiliate (auth user becomes pending affiliate) =====
...
begin
  ...
  insert into public.affiliates (user_id, code, status)
    values (_user, _slug, 'pending')
    returning * into _row;
  return _row;
end;
$$;

-- ===== RPC: attribute referral on signup (called by edge with service role too) =====
create or replace function public.attribute_referral(_referred_user_id uuid, _ref_code text, _ip_hash text default null)
returns boolean
language plpgsql security definer set search_path = public
as $$
declare
  _aff public.affiliates;
begin
  ...
  insert into public.affiliate_referrals (affiliate_id, referred_user_id, ip_hash)
    values (_aff.id, _referred_user_id, _ip_hash);
  return true;
end;
$$;
...
-- ===== RPC: record commission for one-time payment =====
...
declare
  _p record;
  _ref public.affiliate_referrals;
  _aff public.affiliates;
  _plan text;
  _rate numeric;
  _amount numeric;
...
begin
  ...
  select * into _ref from public.affiliate_referrals
    where referred_user_id = _p.user_id and active = true
    limit 1;
  if _ref.id is null then return null; end if;

  select * into _aff from public.affiliates where id = _ref.affiliate_id;
  if _aff.id is null or _aff.status = 'blocked' or _aff.status = 'rejected' then return null; end if;
...
-- ===== RPC: record commission for subscription renewal =====
...
begin
  ...
  select * into _ref from public.affiliate_referrals
    where referred_user_id = _sub.user_id and active = true
    limit 1;
  if _ref.id is null then return null; end if;
  ...
  if _com_id is not null then
    update public.affiliates
      set pending_balance = pending_balance + _amount_com,
          total_earned = total_earned + _amount_com
      where id = _aff.id;
  end if;
  return _com_id;
end;
...
-- ===== RPC: approve commissions older than N days =====
...
begin
  ...
  select coalesce(sum(commission_uyu),0) into _balance from public.affiliate_commissions
    where affiliate_id = _aff.id and status = 'approved';
  ...
  insert into public.affiliate_payouts (affiliate_id, amount_uyu, status, method)
    values (_aff.id, _balance, 'pending', _aff.payout_method)
    returning * into _row;
  update public.affiliate_commissions
    set payout_id = _row.id
    where affiliate_id = _aff.id and status = 'approved' and payout_id is null;
  return _row;
end;
$$;

-- ===== RPC: admin mark payout paid =====
...
declare _p record; _aff_id uuid;
begin
  ...
  update public.affiliates
    set pending_balance = greatest(pending_balance - _p.amount_uyu, 0),
        total_paid = total_paid + _p.amount_uyu
